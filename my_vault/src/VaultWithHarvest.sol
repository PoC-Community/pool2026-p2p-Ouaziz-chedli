...
contract VaultWithHarvest is ReentrancyGuard, Ownable {
    ...
    function deposit(
        ...
        uint256 sharesToMint = _convertToShares(assets);
    ...
    function withdraw(
        ...
        uint256 assetsToWithdraw = _convertToAssets(shares);
        if (shares == 0) revert ZeroAssetsToWithdraw();
        if (assetsToWithdraw == 0) revert ZeroAssetsToWithdraw();
    ...
    function withdrawAll() external nonReentrant returns (uint256 assets) {
        ...
        uint256 assetsToWithdraw = _convertToAssets(userShares);
        asset.safeTransfer(msg.sender, assetsToWithdraw);
        emit Withdraw(msg.sender, assetsToWithdraw, userShares);
        return assetsToWithdraw;
        // OK
        // TODO: Implement
    }
    ...
    function harvest() external nonReentrant returns (uint256 assetReceived) {
        ...
        int256 timeSinceLastHarvest = int256(block.timestamp) -
            int256(lastHarvestTime);
        // TODO: Check cooldown (otherwise revert HarvestCooldownNotMet with remaining time)
        if (timeSinceLastHarvest < int256(harvestCooldown)) {
            int256 timeRemaining = int256(harvestCooldown) -
                timeSinceLastHarvest;
            revert HarvestCooldownNotMet(uint256(timeRemaining));
        }
